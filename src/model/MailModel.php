...
class Mail {    
    ...
    public function envoyerMailer($twig,$destinataire, $sujet,$emailmdp,$code){
        ...
        $mail->Username = $emailmdp['mail']; // Votre adresse email d'envoi        
        $mail->Password = $emailmdp['mdp']; // Le mot de passe de cette adresse email        
        $mail->SMTPSecure = 'tls'; // Accepter SSL        
        $mail->Port = 587;  
        $mail->CharSet = "UTF-8";

        $mail->setFrom($emailmdp['mail'], 'Mailer'); // Personnaliser l'envoyeur        
    ...
    public function envoyerMailerSupprimer($twig,$destinataire, $sujet,$emailmdp){
        $mail = new PHPMailer\PHPMailer\PHPMailer(); 
        $mail->isSMTP(); // Paramétrer le Mailer pour utiliser SMTP    
        $mail->Host = 'smtp.office365.com'; // Spécifier le serveur SMTP        
        $mail->SMTPAuth = true; // Activer authentication SMTP        
        ...
        $mail->isHTML(true); // Paramétrer le format des emails en HTML ou non        
        $mail->Subject = $sujet;        
        $mail->Body = $twig->render("mail/delPersonne_message.html.twig", [ 
            'email' => $destinataire,
        ]);       
